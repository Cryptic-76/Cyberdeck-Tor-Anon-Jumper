...
Gehärtet nach dem EVA-Prinzip mit Anti-Zombie-Handling und expliziten Dateirechten.
"""
...
class PrivoxyManager:
    ...
    def __init__(self) -> None:
        ...
        self._privoxy_log_handle: Optional[TextIO] = None

    def _cleanup_system_privoxy(self) -> None:
        """Stoppt laufende Systemd-Privoxy-Dienste und gibt Port 8118 frei."""
        try:
            subprocess.run(["systemctl", "stop", "privoxy"], capture_output=True, check=False)
            subprocess.run(["service", "privoxy", "stop"], capture_output=True, check=False)
            
            # Prüfe, ob Port noch belegt ist, und beende Alt-Prozesse
            with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as s:
                s.settimeout(0.3)
                if s.connect_ex((PRIVOXY_HOST, int(PRIVOXY_PORT))) == 0:
                    logger.warning("[!] Privoxy-Port 8118 belegt. Räume alte Instanzen auf...")
                    subprocess.run(["pkill", "-9", "-f", "privoxy"], capture_output=True, check=False)
                    time.sleep(0.5)
        except Exception as e:
            logger.debug(f"[*] Hinweis beim Privoxy-Cleanup: {e}")
    ...
    def start_privoxy(self) -> bool:
        """
        Startet Privoxy als direkten Prozess mit der generierten, gehärteten Config aus der RAM-Disk.
        """
        self._cleanup_system_privoxy()
        ...
        try:
            ...
            if self.process.poll() is not None:
                ...
                if self._privoxy_log_handle:
                    self._privoxy_log_handle.close()
                    self._privoxy_log_handle = None
                return False
    ...
    def stop_privoxy(self) -> None:
        ...
        # Systemd/Service-Sicherheitsstopp
        subprocess.run(["systemctl", "stop", "privoxy"], capture_output=True, check=False)

        # Log-Datei-Handle schließen
        if self._privoxy_log_handle is not None:
            try:
                self._privoxy_log_handle.close()
            except Exception:
                pass
